...
import           Elevator.TypeChecker                (typeCheckProg,
                                                      ...
                                                      typeInfer)
import           ElevatorExt.LambdaBox.Compile       (compileProg, compileTerm,
                                                      compileTop, decompileTerm,
                                                      decompileType)
import           ElevatorExt.LambdaBox.ModeSpec      (Mode (..))
import           ElevatorExt.LambdaBox.Parser        (fullCommandParse,
                                                      fullFileParse)
import           ElevatorExt.LambdaBox.PrettyPrinter (prettyType)
import           ElevatorExt.LambdaBox.Syntax        (ElxTop (..))
        ...
        Right (Left top@(ElxDef x _ _)) ->
          case typeCheckProgIncremental iprog (compileTop top) of
            ...
            Left err -> do
              ...
              mainLoop iprog (n + 1)
        Right (Right tm) -> do
          case typeInfer iprog MProg (compileTerm tm) of
            Right (itm, ty) -> do
